...
mint_tokens_random() {
    local contract_address="$1"
    echo -e "${YELLOW}Minting token ke alamat acak di kontrak: $contract_address${RESET}"
    ...
    while IFS= read -r address; do
        ...
        forge create "$SCRIPT_DIR/src/AirdropNode.sol:AirdropNode" \
            --rpc-url "$RPC_URL" \
            --private-key "$PRIVATE_KEY" \
            --constructor-args "$contract_address" "$address" \
            --broadcast || {
            echo -e "${RED}Minting gagal ke alamat $address.${RESET}"
            continue
        }
