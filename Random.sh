...
# Menghasilkan 100 alamat acak menggunakan ethers.js
generate_random_addresses() {
    echo -e "${YELLOW}Menghasilkan 100 alamat acak...${RESET}"
    node -e "
    ...
    " > "$SCRIPT_DIR/random_addresses.txt"
    echo -e "${GREEN}Alamat acak telah disimpan ke random_addresses.txt${RESET}"
}
...
approve_spender() {
    ...
    # Kirim transaksi untuk memberi izin kepada spender
    node -e "
    const { ethers } = require('ethers');
...
# Fungsi minting ke alamat acak
mint_tokens_random() {
    local contract_address="$1"
    local mint_amount=1  # Set jumlah mint menjadi 1 token

    while IFS= read -r address; do
        echo -e "${YELLOW}Minting 1 token ke alamat: $address${RESET}"
        node -e "
        const { ethers } = require('ethers');
        const provider = new ethers.JsonRpcProvider('$RPC_URL');
        const signer = new ethers.Wallet('$PRIVATE_KEY', provider);
        ...
        async function mint() {
            const tx = await contract.mint('$address', $mint_amount);
            console.log('Minting disiarkan:', tx.hash);
        }
        mint();
        "
